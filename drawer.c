...
/*   Updated: 2025/11/03 18:55:22 by nismayil         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fdf.h"

void	draw_pixels(t_coors coors, t_iprops img_props, t_image *img,
		t_breh *breh)
{
	while (1)
	{
		if (coors.x1 >= 0 && coors.x1 < img->width && coors.y1 >= 0
			&& coors.y1 < img->height)
			*(int *)(img_props.data + (coors.y1 * (img_props.size_line)
						+ coors.x1 * (img_props.bpp / 8))) = coors.color;
...
void	draw_line(t_coors coors, t_iprops img_props, t_image *img)
{
	t_breh	breh;
...
void	horizontal_lines(t_map *map, t_space spaces, t_bounds bounds,
		t_img_all img_all)
{
	int		row;
	int		col;
	...
	while (row < map->nrows)
	{
		...
		while (col < map->rows[row].ncols - 1)
		{
			...
			apply_isometric(&coors, &bounds);
			apply_offset(&coors, &spaces);
			coors.color = height_color((coors.z1 + coors.z2) / 2, bounds.min_z,
					bounds.max_z, map->rows[row].cols[col].color);
...
void	vertical_lines(t_map *map, t_space spaces, t_bounds bounds,
		t_img_all img_all)
{
	int		row;
	int		col;
...
void	draw_put_image(t_win *win, t_image *img, t_map *map)
{
	...
	img_props.data = mlx_get_data_addr(img->img_ptr, &(img_props.bpp),
			&(img_props.size_line), &(img_props.endian));
	...
	mlx_put_image_to_window(win->mlx, win->win, img->img_ptr, (win->width
			- img->width) / 2, (win->height - img->height) / 2);
}
