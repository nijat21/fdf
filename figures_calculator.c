...
/*   Updated: 2025/11/03 18:52:07 by nismayil         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
double	calculate_z_scale(t_map *map, t_image *img, int *min_z, int *max_z)
{
	...
	*min_z = min;
	*max_z = max;
	return (fmax(3, (img->height / 6) / ((max - min) + 1)));
}
...
void	get_map_bounds(t_map *map, int w_space, int h_space, t_bounds *bounds)
{
	...
	while (row < map->nrows)
	{
		...
		while (col < map->rows[row].ncols)
		{
			...
			point_apply_isometric(&x, &y, z, (*bounds).z_scale);
		...
		t_space *spaces)
{
	...
	(*spaces).w_space = (int)round(fmax(2, (img->width) / (largest_row(map)
					* 2)));
